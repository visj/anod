...
import {
    ...
    OPT_STABLE, OPT_SETUP
} from '../dist/index.mjs';

/**
 * Global sink — every effect/watch accumulates into this so V8 cannot
 * eliminate the reactive subgraph as dead code. Referenced after run().
 */
let sink = 0;
...
function setupCellx(fw, layers) {
    return fw.withBuild(() => {
        ...
        for (let i = layers; i > 0; i--) {
            ...
            fw.effect(() => { sink += s.prop1.val() })
            fw.effect(() => { sink += s.prop2.val() })
            fw.effect(() => { sink += s.prop3.val() })
            fw.effect(() => { sink += s.prop4.val() })
            fw.effect(() => { sink += s.prop1.val() })
            fw.effect(() => { sink += s.prop2.val() })
            fw.effect(() => { sink += s.prop3.val() })
            fw.effect(() => { sink += s.prop4.val() })
...
const anodBench = {
    ...
    setupCellx(layers) {
        ...
        for (let i = layers; i > 0; i--) {
            ...
            anodEffect(c => { sink += c.read(s.prop1); });
            anodEffect(c => { sink += c.read(s.prop2); });
            anodEffect(c => { sink += c.read(s.prop3); });
            anodEffect(c => { sink += c.read(s.prop4); });
            anodEffect(c => { sink += c.read(s.prop1); });
            anodEffect(c => { sink += c.read(s.prop2); });
            anodEffect(c => { sink += c.read(s.prop3); });
            anodEffect(c => { sink += c.read(s.prop4); });
...
const anodStableBench = {
    setupDeep() {
        ...
        for (let i = 0; i < len; i++) {
            const cSignal = current;
            current = cSignal.derive((_, val) => val + 1);
        }
        current.watch((_, val) => { sink += val; });
    ...
    setupBroad() {
        const head = anodSignal(0);
        for (let i = 0; i < 50; i++) {
            const current = head.derive((c, val) => val + i);
            const current2 = current.derive((c, val) => val + 1);
            current2.watch((_, val) => { sink += val; });
        }
    ...
    setupDiamond() {
        ...
        for (let i = 0; i < width; i++) {
            branches.push(head.derive((c, val) => val + 1));
        }

        const sum = anodCompute(c => {
            return branches.map(x => c.read(x)).reduce((a, b) => a + b, 0);
        }, 0, OPT_STABLE);

        sum.watch((_, val) => { sink += val; });
    ...
    setupTriangle() {
        ...
        for (let i = 0; i < width; i++) {
            const prev = current;
            list.push(current);
            current = prev.derive((c, val) => val + 1);
        }

        const sum = anodCompute(c => {
            return list.map(x => c.read(x)).reduce((a, b) => a + b, 0);
        }, 0, OPT_STABLE);
    ...
    setupMux() {
        ...
        const split = heads
            .map((_, index) => mux.derive((c, val) => val[index]))
            .map(x => x.derive((c, val) => val + 1));

        for (const x of split) {
            x.watch((_, val) => { sink += val; });
        }
    ...
    setupUnstable() {
        const head = anodSignal(0);
        const double = head.derive((c, val) => val * 2);
        const inverse = head.derive((c, val) => -val);
    ...
    setupAvoidable() {
        const head = anodSignal(0);
        const computed1 = head.derive((c, val) => val);
        ...
        computed5.watch((_, val) => { sink += val; });
    ...
    setupRepeatedObservers() {
        ...
        const current = head.derive((c, val) => {
            let result = 0;
            for (let i = 0; i < size; i++) {
                result += val;
            }
    ...
    setupCellx(layers) {
        ...
        for (let i = layers; i > 0; i--) {
            ...
            s.prop1.watch((_, val) => { sink += val; });
            s.prop2.watch((_, val) => { sink += val; });
            s.prop3.watch((_, val) => { sink += val; });
            s.prop4.watch((_, val) => { sink += val; });
            s.prop1.watch((_, val) => { sink += val; });
            s.prop2.watch((_, val) => { sink += val; });
            s.prop3.watch((_, val) => { sink += val; });
            s.prop4.watch((_, val) => { sink += val; });
    ...
    setupMolWire() {
        ...
        G.watch((_, val) => { sink += hard(val); });
        G.watch((_, val) => { sink += val; });
        F.watch((_, val) => { sink += hard(val); });
...
// Prevent V8 from eliminating the entire reactive graph as dead code.
if (sink < 0) {
    console.log(sink);
}
