...
*/
const OVERRIDES_ANOD = {}, W = 1024, Yl = 2048, C = 9792;
...
function setupBroad() {
	const head = wa.signal(0);
	for (let i = 0; i < 50; i++) {
		const current = wa.derive((cx) => {
			counter++;
			return cx.val(head) + i;
		});
		const current2 = wa.derive((cx) => {
			counter++;
...
function setupDiamond() {
	...
	const head = wa.signal(0);
	const branches = [];
	for (let i = 0; i < width; i++) branches.push(wa.derive((cx) => {
		counter++;
		return cx.val(head) + 1;
	}));
	const sum = wa.derive((cx) => {
		counter++;
...
function setupMux() {
	...
	const mux = wa.derive((cx) => {
		counter++;
		return heads.map((h) => cx.val(h));
	});
	const split = heads.map((_, index) => wa.derive((cx) => {
		counter++;
	...
	for (const x of split) wa.watch((cx) => {
		counter++;
		sink += cx.val(x);
	});
	let i = 0;
...
function setupUnstable() {
	const head = wa.signal(0);
	const double = wa.derive((cx) => {
		counter++;
		return cx.val(head) * 2;
	});
	const inverse = wa.derive((cx) => {
		counter++;
		return -cx.val(head);
	});
...
function setupAvoidable() {
	const head = wa.signal(0);
	const computed1 = wa.derive((cx) => {
		counter++;
		return cx.val(head);
	});
	const computed2 = wa.derive((cx) => {
		counter++;
		cx.val(computed1);
		return 0;
	});
	const computed3 = wa.derive((cx) => {
		counter++;
		return cx.val(computed2) + 1;
	});
	const computed4 = wa.derive((cx) => {
		counter++;
		return cx.val(computed3) + 2;
	});
	const computed5 = wa.derive((cx) => {
		counter++;
...
function makeDynGraph(width, totalLayers, staticFraction, nSources) {
	...
	for (let l = 0; l < totalLayers - 1; l++) {
		...
		for (let myDex = 0; myDex < width; myDex++) {
			...
			if (random() < staticFraction) row[myDex] = wa.derive((cx) => {
				counter++;
				let sum = 0;
