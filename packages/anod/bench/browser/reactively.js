...
//#endregion
//#region node_modules/@reactively/core/dist/core.js
let CurrentReaction = void 0;
let CurrentGets = null;
let CurrentGetsIndex = 0;
let EffectQueue = [];
let stabilizeFn = void 0;
...
var Reactive = class {
	...
	get() {
		if (CurrentReaction) if (!CurrentGets && CurrentReaction.sources && CurrentReaction.sources[CurrentGetsIndex] == this) CurrentGetsIndex++;
		else if (!CurrentGets) CurrentGets = [this];
	...
	set(fnOrValue) {
		if (typeof fnOrValue === "function") {
			...
			if (!this.equals(this._value, value)) {
				if (this.observers) for (let i = 0; i < this.observers.length; i++) this.observers[i].stale(2);
				this._value = value;
			}
		}
	}
	stale(state) {
		if (this.state < state) {
			if (this.state === 0 && this.effect) {
				EffectQueue.push(this);
				stabilizeFn?.(this);
			}
			this.state = state;
			if (this.observers) for (let i = 0; i < this.observers.length; i++) this.observers[i].stale(1);
		}
	}
	update() {
		...
		try {
			...
			if (CurrentGets) {
				...
				if (this.sources && CurrentGetsIndex > 0) {
					...
					for (let i = 0; i < CurrentGets.length; i++) this.sources[CurrentGetsIndex + i] = CurrentGets[i];
				} else this.sources = CurrentGets;
		...
		if (!this.equals(oldValue, this._value) && this.observers) for (let i = 0; i < this.observers.length; i++) {
	...
	updateIfNecessary() {
		if (this.state === 1) for (const source of this.sources) {
			source.updateIfNecessary();
			if (this.state === 2) break;
		}
		if (this.state === 2) this.update();
		this.state = 0;
	}
...
//#endregion
//#region bench/reactivity/reactively.js
let sink = 0;
let counter = 0;
const fib = (n) => {
	if (n < 2) return 1;
