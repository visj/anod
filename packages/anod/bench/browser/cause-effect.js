...
//#endregion
//#region node_modules/@zeix/cause-effect/src/util.ts
...
//#endregion
//#region node_modules/@zeix/cause-effect/src/errors.ts
/**
...
*/
var NullishSignalValueError = class extends TypeError {
	/**
...
*/
var UnsetSignalValueError = class extends Error {
	/**
...
*/
var InvalidSignalValueError = class extends TypeError {
	/**
...
*/
var InvalidCallbackError = class extends TypeError {
	/**
...
//#endregion
//#region node_modules/@zeix/cause-effect/src/graph.ts
...
let batchDepth = 0;
let flushing = false;
const DEFAULT_EQUALITY = (a, b) => a === b;
...
function link(source, sink) {
	...
	if (prevSink?.sink === sink && (!isRecomputing || isValidEdge(prevSink, sink))) return;
	const newEdge = {
		source,
		sink,
		nextSource,
...
function propagate(node, newFlag = 2) {
	const flags = node.flags;
	if ("sinks" in node) {
		if ((flags & 3) >= newFlag) return;
		node.flags = flags | newFlag;
		if ("controller" in node && node.controller) {
			node.controller.abort();
			node.controller = void 0;
		}
		for (let e = node.sinks; e; e = e.nextSink) propagate(e.sink, 1);
	} else {
		if ((flags & 3) >= newFlag) return;
		const wasQueued = flags & 3;
		node.flags = newFlag;
		if (!wasQueued) queuedEffects.push(node);
	}
}
...
function runCleanup(owner) {
	...
	if (Array.isArray(owner.cleanup)) for (let i = 0; i < owner.cleanup.length; i++) owner.cleanup[i]();
	else owner.cleanup();
	owner.cleanup = null;
}
function recomputeMemo(node) {
	...
	if (changed) {
		for (let e = node.sinks; e; e = e.nextSink) if (e.sink.flags & 1) e.sink.flags |= 2;
	}
	node.flags = 0;
}
...
function refresh(node) {
	if (node.flags & 1) for (let e = node.sources; e; e = e.nextSource) {
		if ("fn" in e.source) refresh(e.source);
		if (node.flags & 2) break;
	}
	if (node.flags & FLAG_RUNNING) throw new CircularDependencyError("controller" in node ? TYPE_TASK : "value" in node ? TYPE_MEMO : "Effect");
	if (node.flags & 2) if ("controller" in node) recomputeTask(node);
...
function flush() {
	...
	try {
		for (let i = 0; i < queuedEffects.length; i++) {
			...
			if (effect.flags & 3) refresh(effect);
		}
...
//#endregion
//#region node_modules/@zeix/cause-effect/src/nodes/state.ts
/**
...
//#endregion
//#region node_modules/@zeix/cause-effect/src/nodes/memo.ts
function createMemo(fn, options) {
	validateCallback(TYPE_MEMO, fn, isSyncFunction);
	if (options?.value !== void 0) validateSignalValue(TYPE_MEMO, options.value, options?.guard);
	const node = {
		fn,
		value: options?.value,
		flags: 2,
...
//#endregion
//#region node_modules/@zeix/cause-effect/src/nodes/effect.ts
/**
...
//#endregion
//#region bench/reactivity/cause-effect.js
let sink = 0;
let counter = 0;
const fib = (n) => {
	if (n < 2) return 1;
...
function setupDiamond() {
	...
	const head = createState(0);
	const branches = [];
	for (let i = 0; i < width; i++) branches.push(createMemo(() => {
		counter++;
		return head.get() + 1;
	}));
...
function setupMux() {
	...
	const split = heads.map((_, index) => createMemo(() => {
		counter++;
	...
	for (const x of split) createEffect(() => {
		counter++;
		sink += x.get();
	});
	let i = 0;
...
function setupUnstable() {
	...
	const current = createMemo(() => {
		counter++;
		let result = 0;
		for (let i = 0; i < 20; i++) result += head.get() % 2 ? double.get() : inverse.get();
		return result;
	});
	createEffect(() => {
		counter++;
...
function setupRepeatedObservers() {
	...
	const current = createMemo(() => {
		counter++;
		let result = 0;
		for (let i = 0; i < size; i++) result += head.get();
		return result;
	});
	createEffect(() => {
		counter++;
...
function benchCreateSignals(count) {
	return () => {
		let signals = [];
		for (let i = 0; i < count; i++) signals[i] = createState(i);
		return signals;
	};
}
...
function makeDynGraph(width, totalLayers, staticFraction, nSources) {
	...
	for (let i = 0; i < width; i++) sources[i] = createState(i);
	...
	for (let l = 0; l < totalLayers - 1; l++) {
		...
		for (let myDex = 0; myDex < width; myDex++) {
			...
			if (random() < staticFraction) row[myDex] = createMemo(() => {
				counter++;
				let sum = 0;
				for (let s = 0; s < mySources.length; s++) sum += mySources[s].get();
				return sum;
			});
			else {
...
function setupDynBuild(width, totalLayers, staticFraction, nSources) {
	return () => {
		...
		for (let r = 0; r < len; r++) sink += leaves[r].get();
	};
}
/**
...
function setupDynUpdate(width, totalLayers, staticFraction, nSources, readFraction) {
	...
	for (let r = 0; r < allLen; r++) sink += leaves[r].get();
	const rand = pseudoRandom("seed");
	const readLeaves = removeElems(leaves, Math.round(allLen * (1 - readFraction)), rand);
	...
	return () => {
		...
		for (let r = 0; r < readLen; r++) sink += readLeaves[r].get();
	};
}
...
saveRun("cause-effect", await run());
//#endregion
