...
//#endregion
//#region bench/reactivity/preact-signals.js
let sink = 0;
let counter = 0;
const fib = (n) => {
	if (n < 2) return 1;
...
function setupDiamond() {
	...
	const head = y(0);
	const branches = [];
	for (let i = 0; i < width; i++) branches.push(g(() => {
		counter++;
		return head.value + 1;
	}));
	const sum = g(() => {
...
function setupMux() {
	const heads = new Array(100).fill(null).map(() => y(0));
	const mux = g(() => {
		counter++;
		return heads.map((h) => h.value);
	});
	const split = heads.map((_, index) => g(() => {
		counter++;
...
function benchCreateSignals(count) {
	return () => {
		let signals = [];
		for (let i = 0; i < count; i++) signals[i] = y(i);
		return signals;
	};
}
...
function makeDynGraph(width, totalLayers, staticFraction, nSources) {
	...
	for (let i = 0; i < width; i++) sources[i] = y(i);
	...
	for (let l = 0; l < totalLayers - 1; l++) {
		...
		for (let myDex = 0; myDex < width; myDex++) {
			...
			if (random() < staticFraction) row[myDex] = g(() => {
				counter++;
				let sum = 0;
...
saveRun("preact-signals", await run());
//#endregion
