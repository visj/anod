...
describe("entries", () => {
    test("returns an iterator of [index, value] pairs", () => {
        const l = list(["a", "b"]);
        const c = l.entries();
        expect([...c.val()]).toEqual([[0, "a"], [1, "b"]]);
    });

    test("reacts when source changes", () => {
        const l = list(["a"]);
        const c = l.entries();
        expect([...c.val()]).toEqual([[0, "a"]]);
        l.set(["x", "y"]);
        expect([...c.val()]).toEqual([[0, "x"], [1, "y"]]);
    });
});
...
describe("filter", () => {
    ...
    test("reacts when source changes", () => {
        ...
        expect(c.val()).toEqual([2, 3]);
        l.set([5, 6, 7]);
        expect(c.val()).toEqual([5, 6, 7]);
    });

    test("callback receives index", () => {
        const l = list(["a", "b", "c"]);
        const c = l.filter((_, i) => i !== 1);
        expect(c.val()).toEqual(["a", "c"]);
    });

    test("returns empty array when nothing matches", () => {
        const l = list([1, 2, 3]);
        const c = l.filter(() => false);
        expect(c.val()).toEqual([]);
    });
});
...
describe("flatMap", () => {
    test("maps and flattens one level", () => {
        ...
        expect(c.val()).toEqual([1, 2, 2, 4, 3, 6]);
    });

    test("reacts when source changes", () => {
        const l = list([1, 2]);
        const c = l.flatMap((x) => [x, -x]);
        expect(c.val()).toEqual([1, -1, 2, -2]);
        l.set([10]);
        expect(c.val()).toEqual([10, -10]);
    });
});
...
describe("keys", () => {
    test("returns an iterator of indices", () => {
        const l = list(["a", "b", "c"]);
        const c = l.keys();
        expect([...c.val()]).toEqual([0, 1, 2]);
    });

    test("reacts when source changes", () => {
        const l = list(["a"]);
        const c = l.keys();
        expect([...c.val()]).toEqual([0]);
        l.set(["x", "y", "z"]);
        expect([...c.val()]).toEqual([0, 1, 2]);
    });
});
...
describe("map", () => {
    ...
    test("reacts when source changes", () => {
        ...
        expect(c.val()).toEqual([11, 12]);
        l.set([3]);
        expect(c.val()).toEqual([13]);
    });
