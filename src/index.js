	...
	if (flag & Flag.Single) {
		if (found) {
			if (cs.type & Modification.Insertion) {
				if (cs.type & Modification.Ranged) {
			...
			if (index === -1) {
				scope: {
					for (i = 0, len = cs.length; i < len; i++) {
						if (cs[i].type & Modification.Insertion) {
							break scope;
						}
					}
					return index;
				...
				scope: {
					for (i = 0, len = cs.length; i < len; i++) {
						...
						} else if (type & Modification.Indexed) {
							if (index >= cs[i].index) {
								break scope;
							}
						} else {
