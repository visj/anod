...
// export { Val, Func, Signal, Opt, root, dispose, compute, effect, bind, data, value, nil, owner, listener, freeze, peek, cleanup, Data, Value, Computation };
...
// function bind<S extends Val | Val[], T>(src: S, fn: (src: SourceVal<S>, seed: T, prev: typeof NIL | SourceVal<S>) => T): (seed: T) => T {
//     var prev: typeof NIL | SourceVal<S> = NIL;
//     var next: SourceVal<S>;
//     var defer: boolean;
//     var holder: SourceVal<S>;
//     var isArray = Array.isArray(src);
//     var ln = isArray ? (src as Val[]).length : 0;
//     return function(seed: T) {
//         if (prev === NIL) {
//             LISTENER = OWNER as Receive;
//             LISTENER.state |= State.Static;
//             defer = (LISTENER.state & State.Defer) !== 0;
//             if (isArray) {
//                 prev = new Array(ln) as SourceVal<S>;
//                 next = new Array(ln) as SourceVal<S>;
//             }
//         }
//         if (isArray) {
//             for (var i = 0; i < ln; i++) {
//                 next[i] = (src as Val[])[i].val;
//             }
//         } else {
//             next = (src as Val).val;
//         }
//         LISTENER = null;
//         seed = defer ? seed : fn(next, seed, prev);
//         holder = next;
//         next = prev as SourceVal<S>;
//         prev = holder;
//         return seed;
//     }
// }
