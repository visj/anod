...
describe("resource", () => {
    ...
    describe("suspend and controller", () => {
        ...
        test("c.controller() returns AbortController", async () => {
            let ctrl;
            const r = resource(0);
            r.set(1, async (c, prev) => {
                ctrl = c.controller();
        ...
        test("controller aborted on re-activation", async () => {
            let ctrl;
            const r = resource(0);
            r.set(1, async (c, prev) => {
                ctrl = c.controller();
            ...
            expect(firstCtrl.signal.aborted).toBe(true);
        });
    ...
    describe("downstream consumers", () => {
        test("c.pending(resource) returns loading state", async () => {
            root((c) => {
                const r = resource(0);
                r.set(1, async (c, prev) => {
                    return await c.suspend(
        ...
        test("c.rejected(resource) returns error or null", async () => {
            const r = resource(0);
            root((c) => {
                let err = null;
                c.effect((c) => {
            ...
            r.set(1, async (c, prev) => {
                return await c.suspend(Promise.reject(new Error("nope")));
            });
            await settle();
        });

        test("c.suspend(resource) from spawn waits for settle", async () => {
    ...
    describe("integration: optimistic update patterns", () => {
        ...
        test("resource asyncFn awaits a task", async () => {
            await new Promise((done) => {
                root((c) => {
                    const userId = signal(1);
                    const user = c.task(userId, async (id, c) => {
                        return await c.suspend(Promise.resolve({ id, name: "user_" + id }));
                    });
                    const saved = resource(null);

                    settle().then(() => {
                        saved.set({ id: 1, name: "user_1" }, async (c, prev) => {
                            /** Await the task — gets its settled value. */
                            let data = await c.suspend(user);
                            return data;
                        });
                        settle().then(() => {
                            expect(saved.get()).toEqual({ id: 1, name: "user_1" });
                            expect(saved.loading).toBe(false);
                            done();
                        });
                    });
                });
        ...
        test("resource asyncFn awaits settled task and gets its value", async () => {
            await new Promise((done) => {
                root((c) => {
                    const query = signal("a");
                    const results = c.task(query, async (q, c) => {
                        return await c.suspend(
                            new Promise((r) => setTimeout(() => r("result_" + q), 10))
                        );
                    });

                    const cache = resource(null);

                    /** Let initial task settle. */
                    setTimeout(() => {
                        expect(results.get()).toBe("result_a");

                        /** Resource awaits the settled task — gets value immediately. */
                        cache.set("optimistic", async (c, prev) => {
                            let data = await c.suspend(results);
                            return data;
                        });

                        settle().then(() => {
                            expect(cache.get()).toBe("result_a");
        ...
        test("resource asyncFn awaits array of tasks", async () => {
            await new Promise((done) => {
                root((c) => {
                    const taskA = c.task(async (c) => {
                        return await c.suspend(
                    ...
                    const combined = resource(null);
                    combined.set("loading...", async (c, prev) => {
                        let [a, b] = await c.suspend([taskA, taskB]);
                        return a + "+" + b;
                    });

                    expect(combined.get()).toBe("loading...");
                    expect(combined.loading).toBe(true);

                    setTimeout(() => {
                        expect(combined.get()).toBe("A+B");
        ...
        test("resource asyncFn awaits loading task that settles later", async () => {
            await new Promise((done) => {
                root((c) => {
                    let resolveTask;
                    const task = c.task(async (c) => {
                        return await c.suspend(
                    ...
                    const r = resource("initial");
                    r.set("waiting", async (c, prev) => {
                        /** Task is still loading — suspend waits. */
                        let val = await c.suspend(task);
                        return val;
                    });

                    expect(r.get()).toBe("waiting");
                    expect(r.loading).toBe(true);

                    /** Settle the task after a delay. */
                    setTimeout(() => {
                        resolveTask("task_done");
                        settle().then(() => {
                            expect(r.get()).toBe("task_done");
                            expect(r.loading).toBe(false);
                            done();
                        });
                    }, 20);
