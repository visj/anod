...
function createComputation0(i) {
    new Signal.Computed(function () { return i; });
}

function createComputation1(s1) {
    new Signal.Computed(function () {
        return s1.get();
    });
}

function createComputation2(s1, s2) {
    new Signal.Computed(function () { return s1.get() + s2.get(); });
}

function createComputation4(s1, s2, s3, s4) {
    new Signal.Computed(function () { return s1.get() + s2.get() + s3.get() + s4.get(); });
}
...
function updateComputations1to1(n, sources) {
    var s1 = sources[0];
    var c1 = new Signal.Computed(function () { return s1.get(); });
    for (var i = 0; i < n; i++) {
        s1.set(i);
        c1.get();
    }
...
function updateComputations2to1(n, sources) {
    var s1 = sources[0],
        s2 = sources[1];
    var c1 = new Signal.Computed(function () { return s1.get() + s2.get(); });
    for (var i = 0; i < n; i++) {
        s1.set(i);
        c1.get();
    }
...
function updateComputations4to1(n, sources) {
    var s1 = sources[0],
        ...
        s3 = sources[2],
        s4 = sources[3];
    var c1 = new Signal.Computed(function () { return s1.get() + s2.get() + s3.get() + s4.get(); });
    for (var i = 0; i < n; i++) {
        s1.set(i);
        c1.get();
    }
...
function updateComputations1000to1(n, sources) {
    var s1 = sources[0];
    var c1 = new Signal.Computed(function () {
        var sum = 0;
        for (var i = 0; i < 1000; i++) {
            sum += sources[i].get();
        }
...
function updateComputations1to2(n, sources) {
    var s1 = sources[0];
    var c1 = new Signal.Computed(function () { return s1.get(); });
    var c2 = new Signal.Computed(function () { return s1.get(); });
    for (var i = 0; i < n / 2; i++) {
        s1.set(i);
        c1.get();
        c2.get();
...
function updateComputations1to4(n, sources) {
    ...
    var c1 = new Signal.Computed(function () { return s1.get(); });
    var c2 = new Signal.Computed(function () { return s1.get(); });
    var c3 = new Signal.Computed(function () { return s1.get(); });
    var c4 = new Signal.Computed(function () { return s1.get(); });
    for (var i = 0; i < n / 4; i++) {
        s1.set(i);
        c1.get();
        c2.get();
...
function updateComputations1to1000(n, sources) {
    var s1 = sources[0];
    for (var i = 0; i < 1000; i++) {
        new Signal.Computed(function () { return s1.get(); });
    }
    for (var i = 0; i < n / 1000; i++) {
        s1.set(i);
    }
